...
require __DIR__ . '/../../config/database.php'; // Panggil koneksi

// Query untuk mengambil semua data guru
$query = $koneksi->query("SELECT id_guru, nama_guru, nip, kontak FROM guru ORDER BY nama_guru ASC");
?>
...
<div class="overflow-x-auto bg-white rounded-lg shadow">
    <table class="min-w-full divide-y divide-gray-200">
        <thead class="bg-gray-800 text-white">
            <tr>
                <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">ID Guru</th>
            ...
            <?php if ($query->num_rows > 0): ?>
                <?php while ($row = $query->fetch_assoc()): ?>
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4 whitespace-nowrap font-medium"><?= htmlspecialchars($row['id_guru']) ?></td>
                        <td class="px-6 py-4 whitespace-nowrap"><?= htmlspecialchars($row['nama_guru']) ?></td>
                        <td class="px-6 py-4 whitespace-nowrap"><?= htmlspecialchars($row['nip'] ?? '-') ?></td>
                        <td class="px-6 py-4 whitespace-nowrap"><?= htmlspecialchars($row['kontak'] ?? '-') ?></td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <a href="edit.php?id=<?= $row['id_guru'] ?>" class="text-indigo-600 hover:text-indigo-900">Edit</a>
                            <a href="proses.php?aksi=hapus&id=<?= $row['id_guru'] ?>" class="text-red-600 hover:text-red-900 ml-4" onclick="return confirm('Apakah Anda yakin ingin menghapus data guru ini?')">Hapus</a>
                        </td>
                    </tr>
                <?php endwhile; ?>
            <?php else: ?>
                <tr>
                    <td colspan="5" class="text-center py-10 text-gray-500">Tidak ada data guru yang ditemukan.</td>
                </tr>
            <?php endif; ?>
        </tbody>
    </table>
