...
// --- HEADER KOLOM YANG DIPERBAIKI ---
fputcsv($output, ['Tanggal', 'Jam Ke', 'NIP', 'Nama Guru', 'Mata Pelajaran', 'Kelas', 'Status', 'Guru Pengganti', 'Keterangan']);

// --- DATA BARIS YANG DIPERBAIKI ---
while ($row = $result->fetch_assoc()) {
    fputcsv($output, [
        date('d-m-Y', strtotime($row['tanggal'])),
        $row['jam_ke'],
        $row['nip'] ?? '-', // Urutan disesuaikan
        $row['nama_guru_utama'], // Urutan disesuaikan
        $row['nama_mapel'],
        $row['nama_kelas'],
