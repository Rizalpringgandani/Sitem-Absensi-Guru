<?php
$page_title = 'Kelola Data Kelas';
require __DIR__ . '/../templates/header.php';
require __DIR__ . '/../../config/database.php';

$query = $koneksi->query("SELECT id_kelas, nama_kelas FROM kelas ORDER BY nama_kelas ASC");
?>
...
<div class="overflow-x-auto bg-white rounded-lg shadow">
    <table class="min-w-full divide-y divide-gray-200">
        <thead class="bg-gray-800 text-white">
            <tr>
                <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">ID Kelas</th>
        ...
        <tbody class="bg-white divide-y divide-gray-200 text-gray-700">
            <?php if ($query->num_rows > 0): while ($row = $query->fetch_assoc()): ?>
                <tr class="hover:bg-gray-50">
                    <td class="px-6 py-4 whitespace-nowrap font-medium"><?= htmlspecialchars($row['id_kelas']) ?></td>
                    <td class="px-6 py-4 whitespace-nowrap"><?= htmlspecialchars($row['nama_kelas']) ?></td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <a href="edit.php?id=<?= $row['id_kelas'] ?>" class="text-indigo-600 hover:text-indigo-900">Edit</a>
                        <a href="proses.php?aksi=hapus&id=<?= $row['id_kelas'] ?>" class="text-red-600 hover:text-red-900 ml-4" onclick="return confirm('Apakah Anda yakin ingin menghapus data ini?')">Hapus</a>
                    </td>
                </tr>
            <?php endwhile; else: ?>
                <tr>
                    <td colspan="3" class="text-center py-10 text-gray-500">Tidak ada data kelas.</td>
                </tr>
            <?php endif; ?>
        </tbody>
    </table>
